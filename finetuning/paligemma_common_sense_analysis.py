...
class HateExplainDataset(Dataset):
    def __init__(self, csv_file, img_dir, split="train"):
        ...
        for index, item in self.dataset.iterrows():
            dataset_entry = {
                "img": item['image_id'],
                "explanation": item["description_with_commonsense_parameters"].split("Commonsense Analysis:")[
                    -1].strip()
            }
            if index <= 3:
    ...
    def __getitem__(self, idx):
        ...
        prompt = f"Generate common sense analysis of the given meme for intervention."  # target aware
...
if __name__ == "__main__":
    ...
    model.save_pretrained("meme-common-sense-analysis-model/prod")
    processor.save_pretrained("meme-common-sense-analysis-model/prod")

    # Push to Hugging Face Hub
    model.push_to_hub("nrizwan/common-sense-analysis-model")
    processor.push_to_hub("nrizwan/common-sense-analysis-model")
