...
login(token="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")
...
class MAMIDataset(Dataset):
    def __init__(self, base_folder, split):
        ...
        with open(dataset_path, 'r') as tsv_file:
            ...
            # If split is 'test', skip first 809 rows
            if split.lower() == 'test':
                for _ in range(900):
                    next(reader, None)
...
def load_json_folder(folder_path, key="img_fname"):
    """
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for entry in entries:
                    ...
                    if k is not None:
                        ...
                        data_map[k] = entry.get("generated_caption")
    return data_map
...
def load_caption_json_folder(folder_path, key="img_fname", value="generated_caption"):
    """
    Load all JSON files in a folder into a dictionary indexed by the specified key.
    Each file is assumed to contain a JSON array of objects.
    """
...
def load_explanation_json_folder(folder_path, key="img_fname"):
    """
    Load all JSON files in a folder into a dictionary indexed by the specified key.
    Each file is assumed to contain a JSON array of objects.
    """
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for entry in entries:
                    ...
                    if k is not None:
                        # if k in data_map:
                        #     raise ValueError(f"Duplicate key '{k}' found in {file_path}")
                        data_map[k] = entry.get("generated_explanation")
    return data_map
...
generated_captions_map = load_caption_json_folder(generated_captions_folder, key="img_fname")
generated_interpretations_map = load_explanation_json_folder(generated_explanations_folder, key="img_fname")
generated_intervention_map = load_caption_json_folder(generated_intervention_folder, key="img", value="intervention")
...
class PixtralInference:
    ...
    def generate_output(
        ...
        messages.append({
            "role": "user",
            "content": [
                {"type": "image_url", "image_url": self._get_image(image_metadata["img"], base_folders)},
                {"type": "text", "text": f"\nCaption of the image: {caption_img}"},
                {"type": "text", "text": f"""\nExtracted text from the image: ```{image_metadata['text']}```"""},
            ],
        })
        try:
            outputs = self.checkpoint.llm.chat(messages=messages, sampling_params=self.checkpoint.sampling_params)
            print(outputs[0].outputs[0].text)
            # Preprocess input
            generated_text = outputs[0].outputs[0].text
            output_file.write(generated_text + "\n")
        except Exception as e:
            output_file.write("**************Assertion Error************" + "\n")
    ...
    def generate_few_shot_prompts(self, selected_images, dictionary, base_folders):
        messages = [
            {
                ...
                "content": [
                    {"type": "text", "text":
                        "You are an AI assistant tasked with classifying memes into misogynistic or not-misogynistic along with an explanation while also generating an intervention only for misogynistic memes based on the input image, image caption and extracted text obtained from the image.\n"
                        "Consider the following definitions.\n"
                        "1. misogynistic - a meme is misogynous if it conceptually describes an offensive, "
                        ...
                        "Classify the meme as misogynistic or not-misogynistic and then explain the reason based on the above definitions considering the image, provided caption of the image, "
                        "as well as the extracted text from the image delimited by three backticks. If the meme is misogynistic then also generate an intervention for it.\n"
                        "Example output for misogynistic meme : misogynistic - explanation in 30 words that why the meme is misogynistic. Intervention - intervention in 30 words for the misogynistic meme.\n"
                        "Example output for not misogynistic meme : not-misogynistic - explanation in 30 words that why the meme is not-misogynistic."
                    }
                ],
            }
        ]
        ...
        if selected_images:
            for example in selected_images[0]:
                caption_img = generated_captions_map.get(example["img"], "")
                interpretation_img = generated_interpretations_map.get(example["img"], "")
                ...
                messages.append({
                    "role": "user",
                    "content": [
                        {"type": "image_url", "image_url": self._get_image(example["img"], base_folders)},
                        {"type": "text", "text": f"\nCaption of the image: {caption_img}"},
                        {"type": "text", "text": f"""\nExtracted text from the image: ```{example['text']}```"""},
                    ],
                })
                intervene = ""
                if example["label"] == 1 and example["img"] in generated_intervention_map:
                    intervene = "\nIntervention - " + generated_intervention_map[example["img"]]
                
                messages.append({
                    "role": "assistant",
                    "content": [
                        {"type": "text", "text": f"{dictionary[example['label']]} - {interpretation_img}{intervene}"}
                    ],
                })
        return messages
    ...
    def _get_image(self, image_path, base_folders):
        """
        Returns a dictionary that vLLM can accept for an image.
        """
        for base_folder in base_folders:
            full_path = os.path.join(base_folder, image_path)
            if os.path.exists(full_path):
...
class PerformInference:
    def __init__(
        ...
        self.checkpoint_initializer = PixtralCheckpointInitializer(model_checkpoint)
...
class PixtralCheckpointInitializer:
    def __init__(self, model_checkpoint):
        
        self.sampling_params = SamplingParams(max_tokens=50, temperature=0.001)
        
        # Initialize LLM safely with limited memory usage and cores
        self.llm = LLM(
            model=model_checkpoint,
            tokenizer_mode="mistral",
            limit_mm_per_prompt={"image": 9},
            max_model_len=65536
            # engine_args={"num_cores": 1}  # limit multiprocessing
        )

        # Initialize inference class
        self.inference = PixtralInference(self)

    def update_inference_class(self):
        """Update the inference instance if needed."""
        self.inference = PixtralInference(self)
...
if __name__ == "__main__":
    ...
    for embedding in [EMBEDDINGS[2]]:
        ...
        #     base_folders=["./experiments/facebook-hateful-memes/archive (1)/hateful_memes/hateful_memes"],
        #     split="test_seen",
        #     output_pickle_file_name="FHM.pkl",
        #     use_rices_feature=True,
        #     embed_details=embedding_details,
        #     model_checkpoint="mistralai/Pixtral-12B-2409"
        # )
        
        perform_inference = PerformInference(
            dataset_tag="MAMI_dataset",
            base_folders=["/home/subhankar-am/nrizwan/experiments/mami-memes/mami-dataset"],
            split="test",
        ...
        #     base_folders=["/home/subhankar-am/nrizwan/experiments/harm-c-memes-dataset", "/home/subhankar-am/nrizwan/experiments/harm-p-memes-dataset"],
        #     split="test",
        ...
        perform_inference.dynamically_update_inference_class()
        for shot in [8]:
            file_name = f"pixtral_RICES_{embedding}_{shot}_shots_mami_test_seen_last_part"
            
            perform_inference.generate_output(
                output_directory="/home/subhankar-am/nrizwan/experiments/pixtral_explanation_intervention",
            ...
            ## email_classification_report('./experiments', f"{file_name}_0.txt", "subhankar.official185@gmail.com")
