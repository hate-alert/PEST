...
# def load_caption_json_folder(folder_path, key="img_fname"):
#     """
        ...
        if selected_images:
            for i in range(len(selected_images[0])):
                ...
                messages.append({
                    "role": "user",
                    "content": [
                        {"type": "image_url", "image_url": self._get_image(selected_images[0][i]["img"], base_folders)},
                        {"type": "text", "text": f"\nCaption of the image: {caption_img}"},
...
class PixtralCheckpointInitializer:
    def __init__(self, model_checkpoint):
        ...
        self.sampling_params = SamplingParams(max_tokens=100, temperature=0.001)
...
if __name__ == "__main__":
    ...
    for embedding in [EMBEDDINGS[0]]:
        ...
        perform_inference.dynamically_update_inference_class()
        for shot in [2, 4, 8]:
            file_name = f"pixtral_RICES_{embedding}_{shot}_shots_FHM_test_seen"
            ...
            # email_classification_report('/home/subhankar-am/nrizwan/experiments/pixtral_explanation', f"{file_name}_0.txt", "subhankar.official185@gmail.com")
